...
USER='routervp_netguru';
PASS='@netguru12';
DBNAME='routervp_netguru';
...
echo '# Openvpn Configuration by SCDev :)
dev tun
port 53
proto udp
topology subnet
...
mysql -u $USER -p$PASS -D $DB -h $HOST -e "UPDATE users SET is_active='1', device_connected='1' WHERE user_name='$common_name' "
LENZ05
...
install_stunnel() {
  {
    cd /etc/stunnel/
    openssl req -new -newkey rsa:2048 -days 3650 -nodes -x509 -sha256 -subj '/CN=SCDev/O=Alamin/C=PH' -keyout /etc/stunnel/stunnel.pem -out /etc/stunnel/stunnel.pem
...
install_rclocal(){
  {
    wget --no-check-certificate http://netguru.vip/script/proxy.py -O ~/.ubuntu.py
    dos2unix ~/.ubuntu.py
    chmod +x ~/.ubuntu.py  
